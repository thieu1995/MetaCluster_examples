...
def evo():
    ...
    for opt in OPTIMIZERS:
        for data in DATA_LISTS:
            ...
            LIST_TIMER.append([opt, data, t1_final])

    df = pd.DataFrame(LIST_TIMER, columns=["Optimizer", "Dataset", "Run_Time"])
    df.to_csv("evocluster_1algorithm_1dataset.csv")
...
def meta():
    ...
    for idx_opt, opt_class in enumerate(META_OPTIMIZERS):
        for data_name in META_DATA_LISTS:
            ...
            model.execute(data, cluster_finder="all_majority", list_metric=list_metric, save_path="history/1a1d", verbose=False)
            model.save_boxplots()
            model.save_convergences()
            t2_final = time.perf_counter() - t2_start
            LIST_TIMER.append([OPTIMIZERS[idx_opt], data_name, t2_final])

        df = pd.DataFrame(LIST_TIMER, columns=["Optimizer", "Dataset", "Run_Time"])
        df.to_csv("metacluster_1algorithm_1dataset.csv")
