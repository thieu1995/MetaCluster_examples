...
def evo():
    ...
    LIST_TIMER.append(["all", t1_final])

    df = pd.DataFrame(LIST_TIMER, columns=["Dataset", "Run_Time"])
    df.to_csv("evocluster_Nalgorithm_Mdataset.csv")
...
def meta():
    ...
    for data_name in META_DATA_LISTS:
        ...
        model.execute(data, cluster_finder="all_majority", list_metric=list_metric, save_path="history/NaMd", verbose=False)
        model.save_boxplots()
        model.save_convergences()
    t2_final = time.perf_counter() - t2_start
    LIST_TIMER.append(["all", t2_final])

    df = pd.DataFrame(LIST_TIMER, columns=["Dataset", "Run_Time"])
    df.to_csv("metacluster_Nalgorithm_Mdataset.csv")
